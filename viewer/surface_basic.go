...
// SurfaceBasic is the GUI and shows the plots and graphs.
// It receives commands for graphing and viewing various graphs.
type SurfaceBasic struct {
	window   *sdl.Window
	surface  *sdl.Surface
...
// NewSurfaceBasic creates a new viewer and initializes it.
func NewSurfaceBasic() api.ISurface {
	v := new(SurfaceBasic)
	v.opened = false
	v.mod = 200
	return v
...
func (v *SurfaceBasic) SetFont(fontPath string, size int) error {
	var err error
...
func (v *SurfaceBasic) filterEvent(e sdl.Event, userdata interface{}) bool {
	switch t := e.(type) {
	case *sdl.QuitEvent:
		v.running = false
...
func (v *SurfaceBasic) Run() {
	...
	// c := color.RGBA{R: 255, G: 255, B: 255, A: 8}
	// c.B = 0
	// c.A = 255
	...
	v.surface.SetAlphaMod(0)
	v.surface.SetBlendMode(sdl.BLENDMODE_BLEND)
	// println(v.surface.Format.BytesPerPixel)
	for v.running {
		frameStart = time.Now()
		...
		v.surface.FillRect(&rect, sdl.MapRGB(v.surface.Format, 255, 127, 127))
		v.surface.FillRect(&sdl.Rect{X: 50, Y: 50, W: 25, H: 25}, sdl.MapRGBA(v.surface.Format, 255, 255, 255, 50))
		// v.surface.Set(50, 50, c)
...
func (v *SurfaceBasic) initialize() {
	var err error
...
func (v *SurfaceBasic) clearDisplay() {
	v.renderer.SetDrawColor(255, 127, 127, 255)
	v.renderer.Clear()
	// v.window.UpdateSurface()
}
...
// SetDrawColor --
func (v *SurfaceBasic) SetDrawColor(color sdl.Color) {
	v.renderer.SetDrawColor(color.R, color.G, color.B, color.A)
}

// SetPixel --
func (v *SurfaceBasic) SetPixel(x, y int) {
